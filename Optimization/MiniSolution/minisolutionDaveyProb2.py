...
dcLimit = str(raw_input("Please enter the a DC limit: "))
...
costInfo = getDBData("CALL `spLimitDCStoreCombos`(%s);"%(dcLimit),cnx)


start_time = datetime.datetime.now()
dvars = {}
for i in range(len(costInfo)):
    dvars[(costInfo[i][0],costInfo[i][1])] = m.addVar(vtype=GRB.BINARY,obj=costInfo[i][2],name='x_'+str(costInfo[i][0])+"_"+str(costInfo[i][1]))
